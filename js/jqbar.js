        ...
        jqbar: function (options) {
            ...
            return this.each(function () {
                ...
                function animateProgressBar(progressBar) {
                    ...
                    if (level > 30) {
                        level = 30;
                        alert('max value cannot exceed 30 percent');
                    }
                    var w = settings.barLength * level / 30;

                    if (settings.orientation == 'h') {
                        progressBar.animate({ width: w }, {
                            duration: 2000,
                            step: function (currentWidth) {
                                var percent = parseInt(currentWidth / settings.barLength * 30);
                                if (isNaN(percent))
                                    percent = 0;
                        ...
                        var h = settings.barLength - settings.barLength * level / 30;
                        progressBar.animate({ top: h }, {
                            duration: 2000,
                            step: function (currentValue) {
                                var percent = parseInt((settings.barLength - parseInt(currentValue)) / settings.barLength * 30);
                                if (isNaN(percent))
                                    percent = 0;
